...
<div style="font-family:system-ui,-apple-system,sans-serif;max-width:520px;color:#111;line-height:1.6">
  ...
  <p style="margin-top:28px;font-size:12px;color:#999;border-top:1px solid #eee;padding-top:14px">
    ${review ? "Ricontrolla la capienza prima di confermare. Se non disponibile, concorda un altro orario o il rimborso." : "Ricordati di segnare l\u2019appuntamento in Teamup: il collegamento legge la disponibilit\xE0, ma non crea eventi in agenda."}
  </p>
</div>`.trim();
...
async function startCheckout(request, env, provider) {
  ...
  if (service.requiresAppointment) {
    ...
    try {
      ...
      return bad("Non possiamo confermare questo orario. Aggiorna le disponibilit\xE0 e riprova.", env, request, 409);
    }
  ...
  try {
    ...
    if (managed) return bad("Il pagamento non \xE8 stato avviato correttamente. Contatta lo studio prima di riprovare.", env, request, 502);
...
async function adminRoutes(request, env, url, path) {
  ...
  if (path === "/api/admin/teamup/settings") {
    try {
      ...
      return bad(error instanceof SyntaxError ? "Richiesta non valida" : error.message, env, request);
    }
  }
  ...
  async fetch(request, env, ctx) {
    ...
    if (request.method === "GET" && path === "/api/slots") {
      ...
      if (service === TEAMUP_SERVICE) {
        try {
          ...
          return bad("Disponibilit\xE0 temporaneamente non verificabile. Riprova tra poco o contatta lo studio.", env, request, 503);
        }
      }
    ...
    if (path.startsWith("/api/admin/")) {
      try {
        ...
        if (String(error.message).includes("TEAMUP_SLOT_CONFLICT")) return bad("Orario occupato da una prenotazione del sito: scegli un altro orario.", env, request, 409);
        throw error;
      }
    }
