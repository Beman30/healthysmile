...
__name(palmiaHours, "palmiaHours");
...
function automaticWindows(events, date, palmiaId) {
  ...
  if (openings.length) {
    ...
    const append = /* @__PURE__ */ __name((a, b) => {
...
var apiDate = /* @__PURE__ */ __name((ms) => new Date(ms).toISOString().replace(/\.\d{3}Z$/, "+00:00"), "apiDate");
...
async function request(env, method, path, body, version, fetcher = fetch) {
  ...
  try {
    ...
    if (!r.ok) {
      ...
      const id = candidates.find((x) => typeof x === "string" && /^[a-z][a-z0-9_-]{0,95}$/.test(x) && ![token2, key(env)].some((secret) => x.includes(secret) || secret.includes(x)));
      ...
      if (typeof body?.title === "string" && body.title.startsWith("PROVA TECNICA SITO")) {
        ...
        for (const secret of [token2, key(env)]) detail = detail.split(secret).join("[omesso]");
    ...
    try {
      ...
      throw writeError("TEAMUP_RESPONSE_NOT_JSON");
    }
  } catch (e) {
    if (e.teamupCode) throw e;
    throw writeError(controller.signal.aborted ? "TEAMUP_TIMEOUT" : "TEAMUP_NETWORK");
  } finally {
    clearTimeout(timer);
  }
}
...
async function createOwnedHold(env, b, config, fetcher = fetch) {
  ...
  const payload = {
    ...
    remote_id: marker,
    title: b.teamup_test ? "PROVA TECNICA SITO \u2014 da rimuovere" : "Sito \xB7 in attesa di pagamento \xB7 " + b.booking_id.slice(0, 8),
    start_dt: apiDate(start),
    end_dt: apiDate(start + 36e5),
    all_day: false,
  ...
  try {
    ...
    if (!identity(event, row) || Date.parse(event.start_dt) !== start || Date.parse(event.end_dt) !== start + 36e5) throw writeError(!identity(event, row) ? "TEAMUP_EVENT_IDENTITY_MISMATCH" : "TEAMUP_EVENT_TIME_MISMATCH");
  ...
  try {
    ...
    if (action === "delete") {
      ...
      const payload = {
        ...
        title: b.teamup_test ? "PROVA TECNICA SITO \u2014 modifica verificata" : `Sito \xB7 ${b.last_name} ${b.first_name} \xB7 Visita + Igiene`,
        notes: b.teamup_test ? "Prova tecnica senza paziente o pagamento." : `Prenotazione sito ${b.booking_id}
Telefono: ${b.phone}
Acconto ricevuto.`,
        location: event.location,
        who: b.teamup_test ? "Prova tecnica - nessun importo" : Number.isFinite(Number(b.balance_due)) && Number(b.balance_due) > 0 ? "Saldo in studio: " + Number(b.balance_due).toFixed(2) + " EUR" : "Pagamento ricevuto - nessun saldo",
        custom: event.custom
      };
...
async function testWrite(env, config) {
 ...
 step TEXT NOT NULL, code TEXT, updated_at TEXT NOT NULL)`).run();
  ...
  if (previous && !previous.busy && previous.step === "create" && /^TEAMUP_HTTP_400(?:_|$)/.test(previous.code || "")) {
    ...
    if (held.meta.changes !== 1) throw Error("Prova gi\xE0 in corso.");
    try {
      ...
      await env.DB.prepare("UPDATE teamup_write_probe SET busy=0 WHERE id=1 AND booking_id=?").bind(previous.booking_id).run();
    }
  }
  const probeDate = new Date(Date.now() - 864e5).toLocaleDateString("sv-SE", { timeZone: "Europe/Rome" });
 ...
 SELECT 1 FROM teamup_owned_events WHERE booking_id=teamup_write_probe.booking_id AND state<>'deleted'))`).bind(id, now()).run();
  if (lock.meta.changes !== 1) throw Error("Una prova \xE8 in corso o ha lasciato un evento da verificare. Nessun nuovo evento creato.");
  let step = "access";
  const progress = /* @__PURE__ */ __name(async (value) => {
    step = value;
    await env.DB.prepare("UPDATE teamup_write_probe SET step=?,updated_at=? WHERE id=1 AND booking_id=?").bind(step, now(), id).run();
  }, "progress");
  ...
  try {
    ...
    return { ok: false, step, code, booking_id: id, message: "Prova fermata: " + step + " \xB7 " + code + (e.probeDetail ? " \xB7 " + e.probeDetail : "") + ". La scrittura delle prenotazioni resta disattivata." };
  } finally {
    await env.DB.prepare("UPDATE teamup_write_probe SET busy=0,updated_at=? WHERE id=1 AND booking_id=?").bind(now(), id).run();
  }
}
__name(testWrite, "testWrite");
...
async function adminRoutes(request2, env, url, path) {
  ...
  if (path === "/api/admin/teamup/write-test") {
    if (request2.method !== "POST") return bad("Metodo non consentito", env, request2, 405);
    try {
      return ok(await testWrite(env, await settings(db)), env, request2);
    } catch (e) {
      return bad(e.message, env, request2, 409);
    }
  }
  ...
  if (path === "/api/admin/teamup/settings") {
    try {
      if (request2.method === "GET") return ok({ settings: await settings(db), capabilities: { palmia_schedule: true, write_probe: true } }, env, request2);
