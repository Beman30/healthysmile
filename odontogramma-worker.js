...
function buildExtractionPrompt(listinoAttuale, listinoPiano, esempiAttuale, esempiPiano, protocolli, clinicalDeps, rulePrefs) {
  ...
  // NOTA: implant_abutment_crown scatta SOLO se nel treatment_plan c'è una terapia implantare da fare.
  // NON deve scattare quando "impianto" è descritto come situazione attuale già presente.
  const BASE_RULES = [
    { id:'crown_temporary',           text:'Corona definitiva NEL PIANO TERAPEUTICO → suggerisci provvisorio se assente', enabled: true },
    { id:'root_canal_reconstruction', text:'Endodonzia NEL PIANO TERAPEUTICO → suggerisci ricostruzione post-endo se assente', enabled: true },
    { id:'implant_abutment_crown',    text:'Impianto DA FARE (nel treatment_plan, NON nella situazione attuale) → suggerisci abutment + corona se assenti. QUESTA REGOLA NON SCATTA se il dentista descrive un impianto già presente come situazione attuale.', enabled: true },
    { id:'implant_tac',               text:'Caso implantare DA FARE → TAC una sola volta (CASE_LEVEL), mai per dente. NON aggiungere TAC per impianti già presenti.', enabled: true },
...
════ REGOLA FONDAMENTALE: STATO ATTUALE ≠ TERAPIA DA FARE ════

Le seguenti parole/condizioni, se dette SENZA indicatori espliciti di trattamento futuro,
descrivono SOLO la situazione attuale del paziente e NON vanno mai inserite in terapie_da_attuare,
treatment_plan o quote_items:
  - sano
  - mancante
  - impianto
  - otturazione semplice
  - otturazione complessa
  - otturazione di prima classe / II classe / I cl. / II cl.
  - corona
  - ponte
  - devitalizzato
  - ricostruzione presente
  - protesi presente

Esempi di corretta interpretazione:

  "Elemento 14 otturazione complessa di seconda classe"
  → situazione_attuale.denti["14"] = otturazione complessa II classe presente
  → current_situation include il dente 14
  → NON aggiungere nulla in terapie_da_attuare
  → NON aggiungere nulla in treatment_plan
  → NON aggiungere nulla in quote_items

  "Elemento 16 impianto"
  → situazione_attuale = impianto presente
  → NON proporre automaticamente abutment, corona o altri lavori
  → NON aggiungere TAC
  → La regola implant_abutment_crown NON scatta per impianti già presenti

  "Elemento 18 mancante"
  → situazione_attuale = mancante
  → NON proporre impianto, ponte o protesi salvo indicazione esplicita del dentista

  "Elemento 26 sano"
  → situazione_attuale = sano
  → nessuna terapia da inserire

Una terapia/prestazione da fare deve essere creata SOLO se nella frase compaiono
indicatori espliciti come:
  - da fare / da eseguire / da trattare / da curare
  - da estrarre / estrazione indicata
  - da rifare
  - infiltrata / infiltrazione
  - cariato / carie
  - fratturato / frattura
  - ascesso
  - dolore
...
Regola speciale per frasi colloquiali:
  "Elemento 24 sarebbe un dente da estrarre, lo lasciamo così"
  → L'indicatore "da estrarre" è esplicito e va rispettato.
  → La frase colloquiale "lo lasciamo così" NON cancella l'indicazione clinica.
  → situazione_attuale.denti["24"] = dente compromesso / da estrarre
  → terapie_da_attuare.denti["24"] contiene estrazione
  → treatment_plan contiene estrazione elemento 24
  → quote_items contiene estrazione elemento 24

IMPORTANTE:
  - Non generare suggerimenti_clinici automatici da impianti, denti mancanti,
    corone o otturazioni già presenti nella situazione attuale.
  - Le regole cliniche automatiche (implant_abutment_crown, crown_temporary, ecc.)
    vanno applicate SOLO sulle terapie presenti nel treatment_plan (terapie da fare),
    mai sulla situazione attuale.

Esempio di test obbligatorio — se l'input fosse:
  "Elemento 18 mancante. Elemento 17 mancante. Elemento 16 impianto.
   Elemento 15 impianto. Elemento 14 otturazione complessa di seconda classe.
   Elemento 13 otturazione complessa di seconda classe. Elemento 12 otturazione semplice.
   Elemento 11 otturazione semplice. Elemento 21 otturazione semplice.
   Elemento 22 otturazione semplice. Elemento 23 otturazione semplice.
   Elemento 24 sarebbe un dente da estrarre, lo lasciamo così.
   Elemento 25 otturazione semplice. Elemento 26 sano. Elemento 27 sano. Elemento 28 mancante."

L'output CORRETTO sarebbe:
  - situazione_attuale e current_situation contengono TUTTI i denti 18-28
  - terapie_da_attuare contiene SOLO elemento 24: estrazione
  - treatment_plan contiene SOLO elemento 24: estrazione
  - quote_items contiene SOLO elemento 24: estrazione
  - NESSUNA otturazione inserita come terapia da fare
  - NESSUN impianto/corona/abutment/TAC suggerito per 15, 16, 17, 18, 28

════ FINE REGOLA FONDAMENTALE ════
