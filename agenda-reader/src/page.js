import {visitsTotal} from './visits.js';
import {dateRange,paymentTotal} from './range.js';
...
<h1>Agenda studio · v8</h1><p>Igieni sito, pagamenti e prime visite. Solo lettura.</p>
...
<script>
const visitsTotal=${visitsTotal.toString()};
const dateRange=${dateRange.toString()};
const paymentTotal=${paymentTotal.toString()};
...
function render(d){
 ...
 if(view==='payments'&&d.reports.length){const total=paymentTotal(d.reports);text('h2','Totale '+(total.missing?'parziale ':'')+money(total.amount),$('reports'));text('p','Dal '+d.reports[0].date+' al '+d.reports.at(-1).date+' · Somma degli importi indicati per appuntamento.', $('reports'));if(total.review)text('p',total.review+' voci da verificare, escluse dalla somma.', $('reports'));if(total.missing)text('p',total.missing+' giornate senza dati aggiornati, escluse dalla somma.', $('reports'));}
