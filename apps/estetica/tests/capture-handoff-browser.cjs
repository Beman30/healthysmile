...
(async()=>{
 ...
 try{
  ...
  await inViewport('#accept');await page.locator('#accept').click();
  await visible('#captureHandoff');await inViewport('#captureNextPhase');await inViewport('#compactLevel');
  assert.equal(await page.locator('#captureHandoffTitle').textContent(),'Prima foto acquisita');
  assert(await page.evaluate(()=>activeVisit===0&&current===0&&!pending&&!!stream));
  assert(await page.locator('#captureSavedImage').isVisible());assert(await page.locator('#compactActions').isHidden());
  assert.match(await page.locator('#captureHandoffHint').textContent(),/Lascia il telefono/);
  await shot('before-acquired');
  await page.setViewportSize({width:320,height:568});await page.waitForTimeout(300);await inViewport('#captureNextPhase');await inViewport('#compactLevel');
  await page.locator('#captureRedo').click();await visible('#compactManual');
  assert(await page.evaluate(()=>visits[0].photos.has('front-neutral')),'redo must preserve original until confirmation');
  await page.locator('#compactManual').click();await visible('#accept');await page.locator('#retake').click();
  ...
  await page.locator('#tabBefore').click();await page.locator('#dockStart').click();await visible('#compactManual');
