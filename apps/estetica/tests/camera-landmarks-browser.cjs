...
(async()=>{
 ...
 try{
  ...
  await page.locator('#cameraFacing').selectOption('environment');await page.waitForFunction(()=>!document.getElementById('cameraFacing').disabled);assert.equal(await page.evaluate(()=>cameraRequests.at(-1).video.facingMode.ideal),'environment');await inViewport('#capture');assert(await page.locator('#capture').isEnabled());
  ...
  await page.locator('#tabAfter').click();await visible('#referenceContour');assert.equal(await page.locator('#referenceStyle').inputValue(),'contour');assert(await page.locator('#referenceImage').isHidden());await page.locator('#referenceStyle').selectOption('photo');assert(await page.locator('#referenceContour').isHidden());assert(await page.locator('#referenceImage').isVisible());await page.locator('#referenceStyle').selectOption('contour');await visible('#referenceContour');await page.waitForFunction(()=>!document.getElementById('capture').disabled);await page.locator('#capture').click();await page.locator('#accept').click();await page.locator('#cloudStatus').filter({hasText:'Salvato nel cloud'}).waitFor();
  ...
  assert.deepEqual(errors,[]);console.log('PASS mobile front/rear selection, capture/review, photo-derived contour/ghost switching, 3 paired landmarks with magnifier, add/remove, persistence and reopening.');
