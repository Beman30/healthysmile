...
(async()=>{
 ...
 try{
  ...
  await page.locator('#capturePose').selectOption('1');assert.equal(await page.locator('#capturePose').inputValue(),'1');
  ...
  await page.evaluate(()=>{window.beforeTransitionStream=stream;});
  ...
  await page.evaluate(()=>{onMotion({accelerationIncludingGravity:{x:0,y:9.8,z:0}});window.testMotionTimer??=setInterval(()=>onMotion({accelerationIncludingGravity:{x:0,y:9.8,z:0}}),150);});
  await page.waitForFunction(()=>!document.getElementById('compactShot').disabled,{},{timeout:15000});
  // Live movement guidance must stay visible even when the phone sensor needs reactivation.
  await page.evaluate(()=>{beforeFixture.face.size=.28;clearInterval(window.testMotionTimer);lastLevel=null;});
  await page.waitForFunction(()=>document.getElementById('compactInstruction').textContent.includes('Avvicinati'));
  await inViewport('#compactEnableLevel');await inViewport('#compactLevel');
  await page.evaluate(()=>{beforeFixture.face.size=.35;beforeFixture.face.cx=.55;});
  await page.waitForFunction(()=>document.getElementById('compactInstruction').textContent.includes('Spostati'));
  await page.evaluate(()=>{beforeFixture.face.cx=.5;beforeFixture.face.roll=8;});
  await page.waitForFunction(()=>document.getElementById('compactInstruction').textContent.includes('Inclina'));
  await page.evaluate(()=>{beforeFixture.face.roll=0;onMotion({accelerationIncludingGravity:{x:0,y:9.8,z:0}});window.testMotionTimer=setInterval(()=>onMotion({accelerationIncludingGravity:{x:0,y:9.8,z:0}}),150);});
  await page.waitForFunction(()=>!document.getElementById('compactShot').disabled,{},{timeout:15000});
  await shot('after-full-ready');
