...
(()=>{
 ...
 <div class="zone-feedback" role="status" aria-live="polite"><strong id="zoneInstruction"></strong><p id="zoneCheck"></p></div>
 ...
 const stages=[['Occhi','Confronta gli angoli degli occhi e il ponte del naso. Segui le indicazioni per la posa.'],['Lato sinistro','Sinistra del paziente (a destra nell’immagine). Controlla il profilo senza cercare di annullare i cambiamenti del trattamento.'],['Lato destro','Destra del paziente (a sinistra nell’immagine). Ricontrolla anche gli occhi se muovi la testa.'],['Fronte','Confronta fronte e attaccatura solo come controllo visivo: capelli ed espressione possono cambiare.'],['Viso completo','Ricontrolla tutte le zone insieme prima dello scatto. I passaggi precedenti non bloccano la posizione.']];
 let step=0,center={x:.5,y:.35},ref=null,image=null,face=null,faceURL='',facePatient='',openKey='',generation=0,raf=0,lastDraw=0,manual=false,holding=false;
 ...
 function frame(now){
  ...
  const near=$('patientGuidance').classList.contains('patient-guide-close');
  const check=near?'Posa generale vicina. Conferma tu la zona ingrandita.':step>0?'La posa generale va ricontrollata, anche se hai già esaminato gli occhi.':'La guida valuta la posa generale; lo zoom serve al controllo dei dettagli.';
  if($('zoneCheck').textContent!==check)$('zoneCheck').textContent=check;
 ...
 window.addEventListener('hs-face-guide',({detail})=>{const changed=faceURL!==detail.referenceURL||facePatient!==detail.patientId;face=detail.target;faceURL=detail.referenceURL;facePatient=detail.patientId;if(changed&&dialog.open&&!manual)locate();});
 setInterval(()=>{start.hidden=!valid();if(!cloud.patient||facePatient!==cloud.patient.id){face=null;faceURL='';facePatient='';}if(dialog.open&&(!valid()||openKey!==stateKey()))dialog.close();},350);
})();
