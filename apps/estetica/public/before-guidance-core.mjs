...
export function beforePoseState(face,{level=null,phoneConfirmed=false,screenAngle=0,kind='front'}={}){
 ...
 const phone=phoneConfirmed&&portrait&&(!sensor||(Math.abs(level.roll)<=4&&Math.abs(level.pitch)<=4));
 ...
 checks.eyes=Math.abs(face.roll)>4?bad('Raddrizza la testa'):!Number.isFinite(face.eyeOpen)||face.eyeOpen<.12?bad('Apri gli occhi'):good('Occhi aperti e in piano');
 checks.center=Math.abs(face.cx-.5)>.045?bad(face.cx>.5?'Alla tua destra':'Alla tua sinistra'):Math.abs(face.cy-.36)>.055?bad(face.cy>.36?'Più in alto':'Più in basso'):good('Viso centrato');
 checks.yaw=Math.abs(face.yaw)>5?bad('Gira verso la tua '+(face.yaw>0?'destra':'sinistra')):good('Frontale stimata');
 checks.chin=Math.abs(face.pitch)>6?bad(face.pitch>0?'Solleva il mento':'Abbassa il mento'):good('Mento in posizione');
